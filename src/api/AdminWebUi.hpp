...
<section class="panel" style="margin-top:14px"><h3>Provider setup</h3><div class="setup"><select id="provider" onchange="syncModes()"><option value="codex">Codex</option><option value="antigravity">Antigravity</option><option value="zai">Z.ai</option></select><select id="mode" onchange="syncKeyHint()"></select><input id="providerKey" type="password" autocomplete="off" placeholder="API key (only for API-key modes)"><button onclick="addProvider()">Add provider</button></div><p class="muted">Codex and Antigravity consumer login use the providers' official browser sign-in flows. API keys are sent only to this localhost process and are stored through routerAI CredentialStore.</p></section>
<div class="grid">
...
function syncModes(){let p=$('provider').value,m=$('mode');let values=p==='codex'?[['default','ChatGPT / Codex account']]:p==='antigravity'?[['consumer-cli','Consumer CLI session'],['api-project','Gemini API project']]:[['general-api','General API'],['coding-plan','Coding Plan']];m.innerHTML=values.map(x=>`<option value="${x[0]}">${x[1]}</option>`).join('');syncKeyHint()}
function syncKeyHint(){let p=$('provider').value,m=$('mode').value;$('providerKey').placeholder=(p==='codex'||(p==='antigravity'&&m==='consumer-cli'))?'No key required; login after adding':'API key (optional now; can configure later)'}
...
syncModes();if($('key').value)loadAll();
